...
> [!WARNING]\
> для запуска ```ruby``` использовать ```.gitlab-ci-r.yml``` \
> для запуска ```docker``` использовать ```.gitlab-ci.yml```
...
* запустить плейбук, устанавливающий и регистрирующий 2 раннера:
